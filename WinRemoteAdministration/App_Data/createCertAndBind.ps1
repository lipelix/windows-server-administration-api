...
#If new cert installed sucessfully with the same thumbprint 
    ...
    Write-Host "Remove Certificate binding" -ForegroundColor Cyan 
    $Command = "http delete sslcert ipport=0.0.0.0:443"
    $Command | netsh   

    Write-Host "Add Certificate binding" -ForegroundColor Cyan 
    $Command2 = "http add sslcert ipport=0.0.0.0:443 certhash=$thumbprintB appid={$appId}"
    $Command2 | netsh   
} 
